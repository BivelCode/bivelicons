...
async function run() {
   ...
   * to date, drastically reducing build time after the first run.
   * Extends the base SVGO configuration with shape-to-path conversion and
   * group collapsing, then normalizes each SVG to a minimal canonical
   * structure before writing it to packages/webfonts/optimized/<style>.
   */
  ...
  /**
   * Normalize an SVG string to a minimal, canonical structure:
   * <svg xmlns="http://www.w3.org/2000/svg" viewBox="…">…</svg>
   * No XML declaration, no id or data-name attributes.
   */
  function normalizeSvg(svgString) {
    // Remove XML declaration and leading comments
    const body = svgString
      .replace(/^<\?xml[^?]*\?>\s*/i, '')
      .replace(/^(<!--[\s\S]*?-->\s*)+/i, '');
    // Preserve the original viewBox (or default to 0 0 24 24)
    const vbMatch = body.match(/\bviewBox="([^"]*)"/);
    const viewBox = vbMatch ? vbMatch[1] : '0 0 24 24';
    // Replace the opening <svg> tag with a minimal version
    const normalized = body.replace(
      /<svg[^>]*>/,
